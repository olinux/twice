...
public class MouseCursor extends SimplePanel {
	...
	void down(RemoteMouseDownEvent event) {
		genericEventHandling(event);
		down(event.rightButton);
		show();
	}
	 ...
	 * @param right
	 */
	private void down(boolean rightButton) {
		if (rightButton)
	 ...
	 * @param right
	 */
	void up(RemoteMouseUpEvent event) {
		genericEventHandling(event);
		up(event.rightButton);
		show();
	}
	...
	private void genericEventHandling(RemoteEvent<?> event) {
		l.getElement().getStyle().setLeft(image.getOffsetWidth(), Unit.PX);
		l.getElement().getStyle().setTop(image.getOffsetHeight() / 2, Unit.PX);
		String userName = event.getUserName();
		// Set pointer annotation with the two first letters of the user name displayed in upper case
		if (userName != null) {
			l.setText((userName.length() > 1 ? userName.substring(0, 2) : userName).toUpperCase());
		}
		else
			l.setText("");
