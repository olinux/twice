...
public class NewMultiFocusTextBox extends TextBox {

	private final MultiFocus mf = new MultiFocus();
	private final FlowPanel panel = new FlowPanel();

	public NewMultiFocusTextBox() {
		panel.getElement().getStyle().setDisplay(Display.INLINE_BLOCK);
		panel.getElement().getStyle().setPosition(Position.RELATIVE);
		mf.getElement().getStyle().setPosition(Position.ABSOLUTE);
		mf.getElement().getStyle().setLeft(0, Unit.PX);
		mf.getElement().getStyle().setTop(0, Unit.PX);
		mf.getElement().getStyle().setZIndex(2);
		mf.getElement().getStyle().setBackgroundColor("white");
	}
	...
	private class MultiFocus extends Composite implements HasValue<String> {
		...
		private void repositionCursor(String uuid, String color, int x, int y) {
			Cursor c = getOrCreateCursor(uuid, color == null ? "#000000" : color);
			if (c != null) {
		...
		protected class Cursor extends HTML {
			...
			public void setPosition(int position) {
				...
				this.x = (int) Math.max(0, context.measureText(value == null ? "" : position < value.length() ? value.substring(0, position) : value)
						.getWidth());
				for (Cursor c : cursors.values()) {
					c.show();
				}
		...
		public void setValue(String value) {
			...
			NewMultiFocusTextBox.this.setValue(value);
