...
/**
 * This is the default implementation of the multicursor controller which is - that there is no multi cursor control
 * 
 * @author Oliver Schmid
 * 
 */
public class NoMultiCursorController {

	private static NoMultiCursorController controller;

	protected NoMultiCursorController() {
	}

	/**
	 * @return the multi cursor controller instantiated through deferred binding - by default this
	 */
	public static NoMultiCursorController getInstance() {
		if (controller == null)
			controller = GWT.create(NoMultiCursorController.class);
		return controller;
	}
