...
public class CopyOfSimpleHttpUrlConnectionServletFilter implements Filter {

	private JettyProxy servlet = new JettyProxy();
	...
	public void doFilter(ServletRequest genericRequest,
		...
		if (genericRequest instanceof HttpServletRequest
			...
			response.addCookie(new Cookie(Constants.uuidCookie, request
					.getSession().getAttribute(Constants.uuidCookie).toString()));
			...
			if (url == null || url.isEmpty() || !url.startsWith("http")) {
				...
				if (newProxyBase == null) {
					newProxyBase = (String) request.getSession().getAttribute(
							"lastProxy");
				}
			...
			try {
				result = servlet.loadFromProxy(request, response, url,
						servletPath, parser.getProxyBasePath());
			} catch (UnknownHostException e) {
				// If we get a unknown host exception, we try it with the
				// referer
				String referer = request.getHeader("Referer");
				if (parser.getRefererRelative() != null && referer != null
						&& !referer.isEmpty()) {
					URLParser refererParser = new URLParser(referer,
							Rewriter.getServletPath(referer));
					if (refererParser.getProxyBasePath() != null
							&& !refererParser.getProxyBasePath().isEmpty()) {
						String newUrl = refererParser.getProxyBasePath()
								+ parser.getRefererRelative();
						try {
							result = servlet.loadFromProxy(request, response,
									newUrl, servletPath,
			...
			if (result != null) {
				...
				if (result.getContentType() != null
						&& result.getContentType().contains("text/html")
